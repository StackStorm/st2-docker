...
if [ ! -f ${CONF_DIR}/mongo.env ]; then
  echo "MONGO_HOST=${MONGO_HOST:-st2mongo}" > ${CONF_DIR}/mongo.env
...
if [ ! -f ${CONF_DIR}/postgres.env ]; then
  ...
  echo "POSTGRES_HOST=${POSTGRES_HOST:-st2postgres}" >> ${CONF_DIR}/postgres.env
...
if [ ! -f ${CONF_DIR}/rabbitmq.env ]; then
  ...
  echo "RABBITMQ_HOST=${RABBITMQ_HOST:-st2rabbitmq}" >> ${CONF_DIR}/rabbitmq.env
...
if [ ! -f ${CONF_DIR}/redis.env ]; then
  ...
  echo "REDIS_HOST=${REDIS_HOST:-st2redis}" >> ${CONF_DIR}/redis.env
